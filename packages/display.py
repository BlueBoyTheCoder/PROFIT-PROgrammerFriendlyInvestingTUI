...
from orders import Orders, process_order_data
...
def draw_logging_menu(stdscr):
    ...
    Draws logging menu
    Start draw_working_menu function
    """
    curses.start_color()
    curses.use_default_colors()
    curses.init_pair(CursesColors.POSITIVE_TRADE, curses.COLOR_GREEN, curses.COLOR_BLACK)
    curses.init_pair(CursesColors.NEGATIVE_TRADE, curses.COLOR_RED, curses.COLOR_BLACK)
    curses.init_pair(CursesColors.NORMAL_CURRENT, curses.COLOR_BLACK, curses.COLOR_WHITE)
    curses.init_pair(CursesColors.MARKED, 220, curses.COLOR_BLACK)
    curses.init_pair(CursesColors.MARKED_CURRENT, curses.COLOR_BLACK, 220)
...
def draw_working_menu(stdscr):
    """
    Draw working menu
    Initializes classes objexts for trading, obaining and drawing data
    Imports settings that determine menu look
    Displays chart and widget market data
    In loop refreshes data based on signal send by user
    """
    curses.curs_set(0)
    stdscr.nodelay(True)
    stdscr.timeout(1000)
