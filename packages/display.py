...
def draw_menu(stdscr, y, x, height, width, menu: Menu):
    for i, component in enumerate(menu.get_part_components("menu")):
        ...
        if menu.get_component() == component:
            ...
            stdscr.addstr(y + 2 + 3 * i, x + (width - len(component)) // 2,
                          component)
...
def draw_action_order(stdscr, y, x, height, width, menu: Menu):
    x += 1
    width -= 2
    for i, component in enumerate(menu.get_part_components("order")):
        new_y = int(x + (i // 2) * (width) / 3 +
                    (width / 3 - len(component)) // 2)
        draw_block_border(stdscr, y + 1 + (3 * i) % 6, x + int(
            (i // 2) * width / 3), 3, width // 3)
        if menu.get_component() == component:
            stdscr.attron(curses.color_pair(3))
...
def draw_action_settings(stdscr, y, x, height, width, menu: Menu):
    x += 1
    width -= 2
...
def draw_working_menu(stdscr):
    ...
    while True:
        ...
        draw_menu(stdscr, menu_y, menu_x, menu_height, menu_width, menu)

        match action_button:
            case 0:
                pass
            case 1:
                draw_action_settings(stdscr, action_y, action_x, action_height, action_width, menu)
            case 2:
                draw_action_order(stdscr, action_y, action_x, action_height, action_width, menu)
            case 3:
                pass
            case _:
                pass
